...
public class TableDataServiceImpl extends TableDataServiceAdapter {
    ...
    public void createTable(String tableName, List<CreateTableFeatureVo> characterList, String createUser, CategoryEntity nodeData,String uid,String username,String IsFilter,String IsUpload) {
         ...
         *
         */
        List<LinkedHashMap<String, Object>> res = getFilterDataByConditionsCreateTable(characterList, nodeData);
...
//            System.out.println("数据长度为："+diseaseDatum.size());
    ...
    public boolean checkRepeat(String tablename) {
        String s = tableDataMapper.checkRepeat(tablename);
        if(s == null){
            return false;
        }else {
            return true;
...
//            System.out.println("数据长度为："+diseaseDatum.size());
        ...
        manager.setTablename(tableName);
        manager.setDiseasename(nodeData.getLabel());
        manager.setOperators(node.getUsername());
        manager.setUid(uid);
        manager.setUploadmethod("纳排");
        dataManagerMapper.insert(manager);
    }
    ...
    public List<LinkedHashMap<String, Object>> getFilterDataByConditionsByDieaseId(List<CreateTableFeatureVo> characterList, String uid, String username, String nodeid) {
        ...
        CategoryEntity nodeData = categoryMapper.selectById(nodeid);
        System.out.println("================getFilterDataByConditionsByDieaseId==========================");
...
//        }
        ...
        for (CreateTableFeatureVo createTableFeatureVo : characterList) {
            ...
            FieldManagementEntity fieldManagementEntity = fieldManagementMapper.selectOne(new QueryWrapper<FieldManagementEntity>().eq("feature_name", createTableFeatureVo.getFeatureName()));
...
// 插入 filter_data_info 表信息
        ...
        for (CreateTableFeatureVo createTableFeatureVo : characterList) {
            FilterDataCol filterDataCol = new FilterDataCol();
            BeanUtils.copyProperties(createTableFeatureVo,filterDataCol);
            System.out.println(filterDataCol);
    ...
    @Override
    public List<LinkedHashMap<String, Object>> getFilterDataByConditionsCreateTable(List<CreateTableFeatureVo> characterList,CategoryEntity nodeData) {
        List<CategoryEntity> categoryEntities = categoryMapper.selectList(null); // 查询所有的目录信息
        // 找到所有的宽表节点
        List<CategoryEntity> allWideTableNodes = categoryEntities.stream().collect(Collectors.toList());
        System.out.println("所有的宽表节点："+JSON.toJSONString(allWideTableNodes));
